-- Practice 


1. How many movies have at least 4 e-s or E-s in their titles?
SELECT COUNT(*) FROM film
WHERE title ILIKE '%e%e%e%e%';


2. Write the category names and the number of movies per category.
SELECT COUNT(*), category.name FROM category
JOIN film_category ON film_category.category_id = category.category_id
JOIN film ON film.film_id = film_category.film_id
GROUP BY category.name;


3. Which rating category has the most movies?
SELECT COUNT(*), rating FROM film
GROUP BY rating
ORDER BY COUNT(*) DESC
LIMIT 1;


4. In the movie table, list the 3 movies with the longest and the least replacement cost starting with the character K?
SELECT title, length, replacement_cost FROM film
WHERE title LIKE 'K%'
ORDER BY length DESC, replacement_cost ASC
LIMIT 3;


5. What is the name of the customer who makes the most purchases?
SELECT SUM(amount), customer.first_name, customer.last_name FROM payment
JOIN customer ON customer.customer_id = payment.customer_id
GROUP BY payment.customer_id, customer.first_name, customer.last_name
ORDER BY SUM(amount) DESC
LIMIT 1;








